...
def set_location(page, location_name):
    ...
    search_input = search_container.locator("input._1wkJd")
    search_input.fill(location_name)

    # results_container = page.get_by_test_id("address-search-container")
    first_result = page.get_by_test_id("_11n32").first
    first_result.wait_for(state="visible", timeout=10000)
    first_result.click()

    confirm_btn = page.locator("button.sc-iGgWBj dUvMNA")
...
def run_instamart_flow():
    with sync_playwright() as p:
        ...
        try:
            page.goto("https://www.swiggy.com/instamart", wait_until="networkidle")
...
# # For standalone script execution
# if __name__ == "__main__":
#     run_zepto_flow(product_name="atta", location="Mumbai")